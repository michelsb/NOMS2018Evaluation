...
result_norm_test_writer.writerow(['SDP','Scenario', 'Datagram Type', 'Metric', 'P-Value'])
		...
		for frame_size in frame_sizes:
			...
			try:
		 	...
		 	if frame_size in applications:
 				title_graph = title + " - Application: " + frame_size + " - "+scenario+" VNF(s)"
 			else:
 				title_graph = title + " - Frame Size (bytes): " + frame_size + " - "+scenario+" VNF(s)"
...
# Using the number of VNFs as X-axis (one figure per frame size)
...
for metric in metrics[:-1]:	
	...
	writer_np_metrics[metric].writerow(['Datagram Type','Scenario 1', 'Scenario 3', 'Scenario 5', 'Scenario 7'])
	...
	for frame_size in frame_sizes:			
		...
		if frame_size in applications:
				title_graph = "Application: " + frame_size.upper()
		else:
				title_graph = "Frame Size (bytes): " + frame_size
		fig_num = fig_num + 1
		plt.figure(fig_num)
		...
		ax.set_xlabel("Number of VNFs")
		ax.set_title("")
		ax.set_xticks(x + width)
		ax.set_xticklabels(scenarios)
		ax.legend((rects1[0], rects2[0]), ('LB-OVS', 'LB-only'),bbox_to_anchor=(0., 1.02, 1., .102), loc=3,ncol=2, mode="expand", borderaxespad=0.)
...
for metric in metrics:
	...
	for scenario in scenarios:
		...
		if metric != "packet-loss":
			...
			fig, (ax1, ax2) = plt.subplots(1,2, sharex=False, sharey=False,figsize=(8,10))
			...
			ax1.set_xticklabels(frame_sizes[0:2],fontsize=26)
			ax1.tick_params(labelsize=26)
			ax1.yaxis.tick_right()
			#ax1.set_yticklabels(fontsize=20)
			#ax1.tick_params(axis='y', fontsize=20)
			#ax1.get_yticks().set_fontsize(20)
			ax1.grid(True)

			#ax1.get_yticklabels().set_fontsize(20)
			...
			ax2.set_xticklabels(frame_sizes[2:4],fontsize=26)
			...
			plt.legend((rects21[0], rects22[0]), ('LB-OVS', 'LB-only'), bbox_to_anchor=(-1.1, 1.04, 1.8, .102), loc=3, ncol=2, mode="expand", borderaxespad=0.,prop={'size':24})
			...
			ax1.set_title("")
			ax1.set_xticks(x + width)
			ax1.set_xticklabels(frame_sizes[2:4],fontsize=24)
			#ax1.legend((rects21[0], rects22[0]), ('OVS', 'LB'),bbox_to_anchor=(0., 1.02, 1., .102), loc=3,ncol=2, mode="expand", borderaxespad=0.)
			ax1.grid(True)		

			plt.legend((rects21[0], rects22[0]), ('LB-OVS', 'LB-only'), bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=2, mode="expand", borderaxespad=0.,prop={'size':16})

			#autolabel(ax1,rects21)
			#autolabel(ax1,rects22)		

		fig.text(0.5, 0.01, xlabel, ha='center', fontsize=26)
		fig.text(0.01, 0.5, ylabel, va='center', rotation='vertical', fontsize=26)
		...
		#plt.tick_params(labelsize=20)
		plt.subplots_adjust(left  = 0.08, wspace = .4)
